...
<?php
    ...
    if(mysqli_fetch_row($result) > 0) {
        $query="SELECT codigo_pedido FROM facturas ORDER BY codigo_pedido DESC";
        $result = mysqli_query($connection, $query);

        $row = mysqli_fetch_array($result);
        $codigo_pedido = $row['codigo_pedido'] + 1;
        ...
        for ($i=0; $i < sizeof($_SESSION['datos']); $i++) {
            ...
            $query = "INSERT INTO facturas (id_usuario, id_producto, codigo_pedido, producto, precio, fecha_compra) 
            VALUES ('$id_usuario', '$id_producto', '$codigo_pedido', '$producto', '$precio', CURDATE())";
            $result = mysqli_query($connection, $query);
        }
    } else {
        for ($i=0; $i < sizeof($_SESSION['datos']); $i++) {
            ...
            $query = "INSERT INTO facturas (id_usuario, id_producto, codigo_pedido, producto, precio, fecha_compra) 
            VALUES ('$id_usuario', '$id_producto', '1', '$producto', '$precio', CURDATE())";
            $result = mysqli_query($connection, $query);
        }
    }
?>
