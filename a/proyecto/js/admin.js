                ...
                success: function (response) {
                    ...
                    datos.forEach(dato => {
                        ...
                        <tr inputId="${dato.id}" id="${dato.id_producto}_${dato.id}" class="facturaEncontrada_${dato.id}">
                            ...
                            <td>${dato.producto}</td>
                            <td>${dato.precio}</td>
                            <td><a href="#" class="editarFacturas" style='font-size:17px;border:none;cursor:pointer;color:green;'><i class="fa-solid fa-pen"></i></a></td>
                            <td><button style='background-color:transparent;font-size:15px;border:none;cursor:pointer;color:red;'><i id='papelera' class='eliminarFacturas fa-solid fa-trash'></i></button></td>
                            <td style="display:none;"><button id="limpiarResult">Limpiar Resultados</button></td>                 
                        </tr>`
                    });
            ...
            $.ajax({
                ...
                if(!response.error) {
                    ...
                    template += `<tr style="color:white;" inputIdF="${dato.id}">
                                    <td id="resultadoSearch" style="color:green;">Factura: <a style="color:white;"> ${dato.factura}_${dato.id_usuario}</a></td>
                                    <td id="resultadoSearch"><label id="botonVer" for="btn-modal" style="cursor:pointer;"><i class="fa-regular fa-eye"></i></label></td>
                                </tr>`
                    });
                    $('#resultado').show();
